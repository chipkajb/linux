...
return {
  ...
  modules = {
    ...
    git = function()
      local nr = bufnr()
      -- status_dict can lag behind gitsigns_head (e.g. a diffthis scratch buffer),
      -- so guard it here or concatenating status.head throws.
      local status = vim.b[nr].gitsigns_status_dict
      if not vim.b[nr].gitsigns_head or vim.b[nr].gitsigns_git_status or not status or not status.head then
        return ""
      end
